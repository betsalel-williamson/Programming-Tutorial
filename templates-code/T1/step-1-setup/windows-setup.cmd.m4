...
:: adding would exceed the max length. 
:: MAX_PATH, which is defined as 260
:: If so, then alert the user to edit 
:: the path first then run again.
::
...
:: set tempPath="%USERPROFILE%\AppData\Local\Temp\win-setup-path.txt"
:: (ECHO %PATH%)> %tempPath%
:: FOR %%? IN (%tempPath%) DO ( SET /A strlength=%%~z? - 2 )
:: del %tempPath%
:: if strlength 
