...
int main ( ) {
  ...
  // declare my c_string, or my pointer to point to the text "foo"
  char *c_string = "foo";
  // will print the letter 'f'
  printf( "%-35s%c\n", "First letter of c_string:", c_string[0] );
  // note we need a length of one more than the word we enter
  char c_string_with_array[4];
  // we use index counting numbers that start from 0 and go until length minus 1
  c_string_with_array[0] = 'f';
  c_string_with_array[1] = 'o';
  c_string_with_array[2] = 'o';
  c_string_with_array[3] = '\0';
  // will print the letter 'f'
  printf( "%-35s%c\n", "First letter, string as array:", c_string_with_array[0] );
  // will print the letter 'f'
  printf( "%-35s%c\n", "First letter, pointer access:", *c_string_with_array );

  // with the cubby example
  // each variable will be assigned a cubby spot
  int x = 1234;
  // Look up the different print codes online for
  printf( "%-35s%d\n", "Information in cubby:", x );
  ...
  // %x will print the cubby number or memory address using hex values.
  printf( "%-35s%x\n", "Address of information:", &x );

  long cubby_number_of_x_as_long = ( long ) &x;
  // %x will print the cubby number or memory
  printf( "%-35s%x\n",  "Cubby number in long variable:",
    cubby_number_of_x_as_long );

  // we now declare a pointer
  int* pointer = ( int* ) cubby_number_of_x_as_long;
