...
```C
esyscmd({{sed -n '21p' ../T1/step-4-hello-world/1-Hello-World/main.c | sed 's/^/   /g' | ghead -c -1}})
esyscmd({{sed -n '42p' ../T1/step-4-hello-world/1-Hello-World/main.c | sed 's/^/   /g' | ghead -c -1}})
esyscmd({{sed -n '65,66p' ../T1/step-4-hello-world/1-Hello-World/main.c | sed 's/^/   /g' | ghead -c -1}})
```
...
```Tcl
esyscmd({{sed -n '26,32p' ../T1/step-4-hello-world/2-Test-Hello-World/test.tcl | sed 's/^/   /g' | ghead -c -1}})
```
...
```Tcl
esyscmd({{sed -n '1,51p' ../T1/step-4-hello-world/2-Test-Hello-World/test.tcl | sed 's/^/   /g' | ghead -c -1}})
```
...
```C
esyscmd({{cat ../T1/step-4-hello-world/2-Test-Hello-World/main.c | sed 's/^/   /g' | ghead -c -1}})
```
...
```Makefile
esyscmd({{sed -n '7,28p' ../T1/step-4-hello-world/3-Makefile-Hello-World/makefile | sed 's/^     /   /g' | ghead -c -1}})
```
...
```
$ make
esyscmd({{pushd ../T1/step-4-hello-world/3-Makefile-Hello-World > /dev/null 2>&1 && make clean > /dev/null 2>&1 && make > temp-output.log 2>&1 && popd > /dev/null 2>&1}})dnl
esyscmd({{pushd ../T1/step-4-hello-world/3-Makefile-Hello-World > /dev/null 2>&1 && cat temp-output.log && popd > /dev/null 2>&1}})dnl
```

Lets walk through this step by step:
1. `$ make` is the command we typed in the Shell
1. `esyscmd({{pushd ../T1/step-4-hello-world/3-Makefile-Hello-World > /dev/null 2>&1 && cat temp-output.log | sed -n '1p' | ghead -c -1 && popd > /dev/null 2>&1}})` is the command that make will run to compile our program
1. The following is a chunk that tells us there's something up with the `main` function.
   ```
esyscmd({{pushd ../T1/step-4-hello-world/3-Makefile-Hello-World > /dev/null 2>&1 && cat temp-output.log | sed -n '2p' | sed 's/^/   /g' && popd > /dev/null 2>&1}})dnl
   ```
pushdef({{__err_line_num__}}, {{esyscmd({{pushd ../T1/step-4-hello-world/3-Makefile-Hello-World > /dev/null 2>&1 && cat temp-output.log | perl -lane 'print /main[.]c:(\d+):\d+/' | tr -d '\n' && popd > /dev/null 2>&1}})}})dnl
pushdef({{__err_col_num__}}, {{esyscmd({{pushd ../T1/step-4-hello-world/3-Makefile-Hello-World > /dev/null 2>&1 && cat temp-output.log | perl -lane 'print /main[.]c:\d+:(\d+)/' | tr -d '\n' && popd > /dev/null 2>&1}})}})dnl

   The file that we need to look at is `main.c`, we then need to look at `esyscmd({{pushd ../T1/step-4-hello-world/3-Makefile-Hello-World > /dev/null 2>&1 && cat temp-output.log | perl -lane 'print /(main[.]c:\d+:\d+)/' | tr -d '\n' && popd > /dev/null 2>&1}})` that means the file `main.c`, line `{{}}__err_line_num__{{}}`, column `{{}}__err_col_num__{{}}`. 
...
1. The next few lines are the pretty version of what is at line `{{}}__err_line_num__{{}}`, column `{{}}__err_col_num__{{}}`.
   ```
esyscmd({{pushd ../T1/step-4-hello-world/3-Makefile-Hello-World > /dev/null 2>&1 && cat temp-output.log | sed -n '3,4p' && popd > /dev/null 2>&1}})dnl
   ```
...
```
esyscmd({{sed -n '18p' ../T1/step-4-hello-world/3-Makefile-Hello-World/main-fixed.c | ghead -c -1}})
```
